...
\entry{$\mathbf C_{\mathcal D}$}{Covariance matrix formed using $C$ and $\mathcal D$}
...
%
As a multivariate Gaussian random variable is characterized by a mean vector and covariance matrix, a GP is characterized by a mean and covariance functions $\mu:\mathcal D\to \mathbb R$ and $C:\mathcal D\times \mathcal D\to \mathbb R$, where $D$ is the domain of the process. 
%
...
and $\mathcal D = (\boldsymbol \eta^T,\mathbf y_t^T)^T$.
%
...
\begin{acknowledgment}
	\thanks{
		The authors gratefully acknowledge \textit{please remember to list all relevant funding sources in the unblinded version}}
\end{acknowledgment}
