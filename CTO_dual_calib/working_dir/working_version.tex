...
% Honestly not sure what I'm on about here
%%
%An obvious approach when both calibration and design are undertaken and the calibration parameter is suspected to be functionally dependent on the design settings is to perform the design optimization first over some range of the calibration parameters known to contain their true values, and then the calibration.
%%
...
%%
%In such a situation, the KOH framework for design retains its advantage of interpretable and model-driven uncertainty quantification, and adds another advantage: namely, that DCTO constitutes a method for combining both the observations of the true system and the existing uncalibrated computer model to select new evaluation points.
%%
...
%
The total wall time required for the MCMC to complete DCTO in this case was TODO HOW LONG? (on a laptop with an Intel Core i7-9750H CPU and 16GB of RAM).
%
The posterior distributions of the calibration and design inputs are are shown in Figure \ref{fig:dvs_calib_and_design}.
%
The posterior distribution of the elastic modulus for the system assigns high likelihood to the expected value of 6.2e10.
%
