...
\title{\bf Simultaneous Bayesian calibration and engineering design with an application to a vibration isolation system}
...
%
On the other hand, our combined approach to calibration and design includes uncertainty quantification as an intrinsic aspect of the framework.
%
...
%
Convergence was verified visually and by the Gelman-Rubin statistic ($\approx 1.01$; \if1\diss\citeauthor{Gelman1992a}, \citeyear{Gelman1992a}\else\cite{Gelman1992a}\fi).
%
...
\begin{figure*}
	...
	\if1\diss
		...
		\begin{tabular}{p{.16\linewidth} c c}
	\else
...
%\begin{figure*}
%	\centering
%	\includegraphics[width=0.85\linewidth]{oscillator.jpg}
%	\captionsetup{width=.85\linewidth}
%	\caption{The physical test rig for the dynamic vibration system. The test setup is shown from three views. Depicted here are the rigid mass  (1), one leaf spring (2), VCA (3), fixed leaf spring support (4), mount (5) to suspend the frame (8), acceleration sensor $S_{a;z}$ (6), force sensor $S_{F_\mathrm{VCA}}$ (7), rigid frame (8), and a modal hammer (9) with a force sensor $S_F$ to excite the frame. The sensor $S_{a;w}$ to measure the acceleration of the frame is on the inner side of the frame near the location where the impulse hammer hits ($S_{a;w}$ not visible in the figure).}
...
\begin{figure*}
	\centering
	\includegraphics[width=0.85\linewidth]{schematic.jpg}
	\captionsetup{width=.85\linewidth}
...
%
In the laboratory application, the frame suspends from a rigid mount via elastic straps vertical to the $z$-direction, allowing the frame to move freely in the $z$-direction as shown in Figure \ref{fig:oscillator}c. 
%
...
%
Two leaf springs are mounted in parallel with length $l$ on each side of A-B and B-C (see Figures \ref{fig:oscillator}c and \ref{fig:schematic}c). 
%
...
\begin{table*}
	...
	\renewcommand{\arraystretch}{1.2}% Tighter
	\if1\diss \setlength{\tabcolsep}{1pt} \fi
	\begin{tabular}{lllll}
		\hline
		Category&Property&Variable&Value&Unit\\
		\hline\hline
		\if1\diss
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Rigid frame \\structure\end{tabular}&
		\else
		...
		\hline
		\if1\diss
			\multirow{2}{2.5cm}{Vibrating rigid mass}&
		\else
...
\begin{table*}
	...
	\begin{tabular}{c|ccc}
		...
		Case&
		\if1\diss
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Control \\Parameter $x$\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Design \\Parameter $\theta_d$\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}5-times Avg. \\System Response $y$\end{tabular}\\
		\else
			Control Parameter $x$&Design Parameter $\theta_d$&5-times Avg. System Response $y$\\
		\fi
		\hline\hline
		\if1\diss
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Variable \\unit\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Mass \\(kg)\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Gain factor \\(Ns/m)\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Overall damping ratio\footnotemark \\(-)\end{tabular}\\
		\else
...
\end{table*}
%
\if1\diss \footnotetext{``Overall damping ratio'' refers to the combination of active and passive damping.}\fi
%
...
\begin{figure*}
	\centering
	\includegraphics[width=.45\linewidth]{Figure_9}
	\includegraphics[width=.45\linewidth]{Figure_10}
	\captionsetup{width=.85\linewidth}
...
%
\if0\diss \footnotetext{``Overall damping ratio'' refers to the combination of active and passive damping.}\fi
%
...
\begin{table*}
	...
	\begin{tabular}{c|cccc}
		...
		\if1\diss
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Control \\Parameter $x$\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Calibration \\Parameter $\theta_c$\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Design \\Parameter $\theta_d$\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}System \\Response $y$\end{tabular}\\
			\hline\hline
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Variable \\unit\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Mass \\(kg)\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Elastic Modulus \\(N/m\textsuperscript{2})\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Gain factor \\(Ns/m)\end{tabular}&
			\renewcommand{\arraystretch}{.95}\begin{tabular}{@{}c@{}}Overall damping ratio \\(-)\end{tabular}\\
		\else
