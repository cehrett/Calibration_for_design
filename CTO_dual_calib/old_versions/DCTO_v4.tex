...
\begin{abstract}
	{\it 
		...
		%
		We demonstrate our approach in a dynamic vibration system.
		%
...
%
In Figure \ref{fig:schematic}a, a rigid mass $m$ oscillates in the z-direction due to a base point excitation w(t). A damper with the damping coefficient b and a spring element with a stiffness constant k connect the mass to the base point. 
%
\begin{figure*}
	\centering
...
\end{figure*}
%
The damper and spring provide the system’s internal passive damping force, active damping force, and stiffness force 
%
\begin{equation}
	F_b=b[\dot z(t)-\dot w(t)],\ F_k=k[z(t)-w(t)],\ F_a=-g\dot z(t)
\end{equation}
%
with $F_a$ derived from a simple velocity feedback control with the gain factor $g$.
%

%
The inhomogeneous differential equation of motion of the one-mass oscillator in Figure \ref{fig:schematic}a can be written as
%
...
%
\begin{equation}
	2D_p\omega_0=\frac bm,\ \text{and }\omega_0^2= \frac km
\end{equation}
%
including the damping ratio $D_p$ from passive damping, with $0<D_p<1$, as well as the angular eigenfrequency $omega_0$.
%
The term $\omega_0^2r(t)$ in \eqref{eq:inhomde} is the general expression for the excitation function, which, in this case, is the linear combination of damper and spring base point excitation $2D_p\omega_0\dot w(t) + \omega_o^2w(t)$.
%

%
Figure \ref{fig:schematic}b depicts the representation of the laboratory set-up used in this study, in which a rigid frame with mass $m_f$ serves as a base point structure. 
%
The frame is fixed by a gliding support that is assumed to have no friction perpendicular to the $z$-direction.
%
...
%
As shown in Figure \ref{fig:schematic}c, the frame retains two supports that fix a leaf spring at its ends at A and C, with the effective bending length $l$ on both sides A-B and B-C, with a rigid mass $m$ in the center position at B.
%
The leaf spring is the practical realization of the spring elements in Figure \ref{fig:schematic}a and b. Its
stiffness 
%
\begin{equation}
	k^*=12EI/l^3
\end{equation}
%
is a function of the bending stiffness $EI$, where $E$ is the Young’s modulus of the leaf spring, $I$ is the geometrical moment of inertia, and $l$ is the length of the leaf spring.
%
Two leaf springs are mounted in parallel with length $l$ on each side of A-B and B-C (see Figure \ref{fig:schematic}c). 
%
With four leaf springs, the total stiffness becomes $k = 4k^*$. The two supports at A and C in Figure \ref{fig:schematic}c are adjustable along $l$ to tune the leaf spring’s bending deflection and therefore its effective stiffness $k$.
%
...
\end{equation}
%
in the time domain, with the damping ratio $D_f$, angular eigenfrequency $\omega_{0,f}$ and the damped
angular eigenfrequency $\omega_{0,f}$ of the frame’s movement in $z$-direction. (6) is only valid for low damping $0 < D_f < 1$. This leads to the total vibration response 
%
\begin{equation}
	z(t)=r_0 \{1-e^{-D_{\omega_0}}t[\cos\omega_D t-D\frac{\omega_0}{\omega_D}\sin \omega_Dt]\} .
\end{equation}
%
The \emph{particular solution} $r_0$ is part of the general excitation function $\omega_o^2r(t)$ in (\ref{eq:inhomde}), which takes the form of an excitation step function 
%
\begin{equation}
	r(t) = r_o\sigma(t-t_0)
\end{equation}
%
when multiplied with the unit step function $\sigma(t - t_0)$ as the integral of the Dirac-impulse
function $\delta(t - t_0)$ in (\ref{eq:fhat}). 
%
From the relation $2D_p(\dot\omega_0)\dot w(t)+\omega_0^2w(t)=\omega_0^2r(t)$ in (\ref{eq:inhomde}), 
%
\begin{equation}
	r_0=\frac1{\omega_0}2D_p\dot w_0+w_0
\end{equation}
%
with the velocity $\dot w_0$ and displacement $w_0$ at $t=t_0$ that are derived from (\ref{eq:wt}).
%

%
In this demonstration, the design problem is formulated with the gain factor $g$ being the design parameter; the constant $k$ for the four leaf springs is assumed to be poorly known and assigned as the calibration parameter, and the mass $m$ of the system that needs to be vibration isolated is treated as the control parameter. 
%
The control parameter varies within a predetermined range, the calibration parameter is tuned with respect to the experimental data, and the design parameter is sought during the design process for the optimal vibration isolation outcome.
%
...
\begin{table*}
	...
	\begin{tabular}{c|ccc}
		\hline
		Case&Control Parameter&Design Parameter&5-times avg. system response\\
		\hline\hline
...
\begin{figure*}
	\centering
	\includegraphics[width=0.75\linewidth]{vibration}
	\captionsetup{width=.85\linewidth}
...
%
The damping ratio, a dimensionless measure that describes the damping level, is calculated as follows:
%
\begin{equation}\label{eq:delta}
	\delta = \frac 1n \log \left( \frac{\ddot x(t)}{\ddot x(t+nT)}  \right)
\end{equation}
\begin{equation}\label{eq:D}
	D=\frac1{\sqrt{ 1+\frac{2\pi}\delta }}
\end{equation}
%
where $\ddot x(t)$ is the 1\textsuperscript{st} peak value of mass acceleration, $\ddot x(t + nT)$ is the $(n+1$\textsuperscript{th} peak value of mass acceleration, and $n$ is the number of peak intervals. $\delta$ is here the logarithmic decrement, which is used to compute the damping ratio $D$. By following these two equations, the system responses under various experiment tests are summarized in Table \ref{table:exp}.
%
...
%
Dashpot elements are used for the damper and the gain to model velocity-dependent forces. 
%
...
\begin{table*}
	...
	\begin{tabular}{c|cccc}
		\hline
		Case&Control Parameter&Calibration Parameter&Design Parameter&Numerical System Response\\
		\hline\hline
