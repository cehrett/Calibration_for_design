...
In undertaking model calibration to achieve desired system output, directly setting desired observation is not the only option. For example, in the application considered below in Section \ref{application}, the calibration parameters are controllable features of material design, and the cost of the resulting material is one of the outputs for which we establish targets. Two approaches are used for doing this. One approach is simply to include a desired observation of cost, and calibrate to that along with the other desired observation targets.
...
\subsubsection{Joint calibration}
% Careful how you do this. You probably want to account for model shortcoming by way of observation variance, not discrepancy, since a discrepancy function would confound model shortcoming with model inadequacy 
