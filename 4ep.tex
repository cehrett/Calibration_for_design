...
% offer a Bayesian approach to computer model calibration.
% that allows for the uncertainty of the calibration parameters in the predictions of the resulting calibrated model.
% 
...
%$\boldsymbol \beta^\eta = (\beta^\eta_1,\cdots,\beta_{p+q}^\eta)^T$, and $\boldsymbol \beta^\delta = (\beta^\delta_1,\cdots,\beta_{p+q}^\delta)^T$, we then have the distribution of $\mathcal D$ as 
%%
%\begin{equation}
...
\begin{figure}
\centering
\includegraphics[width=.8\linewidth]{FIG_cost_grid_pareto_bands.eps}
