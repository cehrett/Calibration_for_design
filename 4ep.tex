...
This problem can be alleviated by adding a small nugget to $\mathbf C_{\mathbf X,\mathbf X}$; i.e., we can set $\mathbf C_{\mathbf X,\mathbf X}^\xi= \mathbf C_{\mathbf X,\mathbf X} + \xi \cdot \mathbf I_{\mathrm{dim}(\mathbf X)}$ for some very small value of $\xi$, e.g., $\xi=10^{-4}$. 
