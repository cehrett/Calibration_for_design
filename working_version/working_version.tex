...
%
Common to those approaches is the conception of calibration as using real observations to get a posterior distribution on unknown parameters $\boldsymbol\theta$ so that the posterior predictive distribution of the model approximates reality.
%
By contrast, in our proposed methodology, we use artificial observations (representing design targets) to obtain a posterior distribution on design variables $\boldsymbol\theta$ so that the posterior predictive distribution of the model approaches those targets.
%
...
%
In Section \ref{calib_for_design}, we review the calibration framework that grounds our proposed design optimization approach. 
%
...
%
Section \ref{conclusion} discusses the results and offers thoughts about future directions.
...
%
In situations where one lacks the prior knowledge necessary to select target outcomes near the Pareto front and an accurate prior for $\lambda_\delta$, an alternative is to use a ``preliminary round'' of CTO to estimate the Pareto front.
%
...
%
The resulting posterior predictive distribution can be filtered to retain only its Pareto front, thereby forming an estimate of the true Pareto front in the vicinity of the target outcome.
%
%This preliminary estimate (unlike the estimate achieved after full CTO) does not include uncertainty quantification. 
%
This estimate allows one to select a new set of target outcomes that is known to lie near the optimal region, along with an accurate and informative prior on $\lambda_\delta$.
%
Performing CTO with these new targets and prior will result in a posterior distribution that concentrates on the optimal region, and the resulting posterior predictive distribution will allow one to estimate optimal output with appropriate quantification of the uncertainty.
%
%
...
%
However, firstly, the feasible design space is distant from $(0,0)$, and thus large portions of the design space are roughly as close to $(0,0)$ as is the optimum, leading to poor identifiability of the optimal region.
%
...
%
Such a choice of new target outcome would answer all of the above problems.
%
...
%
Figure \ref{fig:toy_sim_outputs} displays the output surfaces as functions of $\theta_1$ and $\theta_2$ at $x = 2$.
%
\begin{figure}
\centering
...
\end{figure}
%
Assuming an easily evaluated model (so that a GP emulator is not needed), we have
%
$
\mathbf z(x) = \mathbf f(x,\boldsymbol \theta) + \delta(x) + \boldsymbol\epsilon
$
%
for target outcome $\mathbf z$, where $\mathbf f = (y_1,y_2,y_3)^T$ is the model output, $\delta(\cdot)$ is the discrepancy between the optimal region and the target outcome, and $\boldsymbol \epsilon$ is the tolerance and follows a $N(\mathbf 0,0.05I)$ distribution. 
%
...
%
We estimated the Pareto front via a preliminary round of CTO with $\lambda_\delta\sim \mathrm{Exp}(1)$ in order to estimate the standardized distance of the target outcome from the Pareto front.
%
%We filtered the resulting posterior predictive distribution to retain only the Pareto optimal points.
%
Rescaling so that each model output $y$ is replaced with $y^*=(y-\mathbb E(y))/\sqrt{\mathbb V(y)}$, the distance from the Pareto front to $(0,0,0)$ is 16 units.
%
...
%
This confidence is based on the observation that $F(\mathbf x-(0.71,0.71,17.92)^T)>0.95$, where $\mathbf x$ is the nearest point of the estimated Pareto front and $F$ is the cdf of the tolerance $\boldsymbol\epsilon\sim	 N(\mathbf 0, 0.05I)$.
%
...
%
The benefits of preliminary CTO are apparent in the greater spread of the posterior distribution from direct CTO.
%
...
Two primary performance measures for the design of wind turbine blades are tip deflection and twist angle.
%
Here, the goal of engineering design is to keep these measures and the material cost as small as possible.
%
The blade is to be a composite of a given pair of \emph{matrix} and \emph{filler} materials. 
%
In a composite, the matrix holds the filler together; an example would be concrete, in which a filler of loose stones is combined with a matrix of cement.
%
Given a fixed choice of matrix and filler, the properties of the material (and thus the blade performance and cost) depend on the thickness of the shear web used in the blade and on the \emph{volume fraction}, which is the ratio of the filler to matrix.
%
%The resulting material properties impact the performance and cost of the blade. 
%
Temperature also affects the composite material properties and hence performance.
%
The finite element model takes as inputs a triplet $(h,v,k)$, where $h$ is the operating temperature of the turbine (in kelvin), $v$ is the volume fraction of the material, and $k$ is the thickness of the material (in mm). 
%
...
%
The marginal posterior distribution of $\boldsymbol\theta$ is shown in Figure \ref{fig:wt_marg_post} as contours of highest density regions.
%
...
\end{figure}
%
The prior and posterior marginal predictive distributions of the model outputs are shown in Figure \ref{fig:prior_post_pred_comp}, where the prior predictive distributions are based on a uniform sampling of the model inputs.
%
\begin{figure}
\centering
%\includegraphics[width=.4\linewidth]{FIG_post_dist_contourplot}
...
%
Though the mean performance outcomes are approximately the same under the posterior distribution as under the prior, the mean cost per square meter and the uncertainty of the outcomes have both decreased dramatically.
%
...
computer model calibration framework established by \cite{Kennedy2001} can be adapted for engineering design. 
%
Calibration to target outcomes undertakes design by ``calibrating'' a computer model not to field observations, but rather to artificial data representing performance and cost targets for the system. 
%
...
%
The resulting posterior predictive distribution approximates the target outcomes, so that the posterior distribution of $\boldsymbol\theta$ constitutes a distribution on optimal design settings for the system.
%
