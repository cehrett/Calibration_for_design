...
%%%%%%%%
%
Computer model calibration typically operates by choosing parameters in a computer model so that the model’s output faithfully predicts reality. 
%
By using desired performance targets in place of observed data, we show that calibration techniques can be repurposed to wed engineering and material design, two processes that are typically carried out separately.
% 
...
%
Call design goals (such as performance and cost targets) treated as observations in the design procedure we propose below ``target outcomes'', and call that procedure, which uses a Bayesian model calibration framework with target outcomes in place of real observations, ``calibration to target outcomes" (CTO). 
%
