...
%To employ an emulator, suppose that we have inputs $\{(\mathbf x_i,\mathbf t_i)\}_{i=1}^n\subseteq \mathbb R^p\times \mathbb R^q$ scaled to the Cartesian product of the $p-$ and $q-$unit hypercubes, and observations 
%%
...
%Hereafter in this work, we model 
...
%In cases where little is known about the Pareto front of a system prior to calibration, in order to select a performance target and an appropriately informative prior for $\lambda_\delta$ i
%To improve one's choice of performance targets (by decreasing the distance between the targets and the optimal region) and select an accurate prior for $\lambda_\delta$, a preliminary round of CDO can be used to estimate the Pareto front.
...
%
%As described above, one could perform CDO 
%
...
%%
%For instance, consider a case of bivariate positive output $(y_1,y_2)$ where the optimal value of $y_2$ is a continuous and strictly decreasing function of $y_1$, as in Figure \ref{fig:do_selection}.
%%
%There, any value of $y_1$ corresponds to some point in the Pareto front (since any point in the model range cannot be reduced in $y_1$ without increasing the optimal $y_2$), and so it may be unclear to what performance targets the system should be calibrated.
%%
